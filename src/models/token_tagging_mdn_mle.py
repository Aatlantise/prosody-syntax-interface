...
from transformers import AdamW, AutoModel, GPT2Model, AutoConfig, AutoModelForCausalLM, GPT2Config, BertConfig, BertModel, get_linear_schedule_with_warmup, AutoTokenizer
import numpy as np
...
class TokenTaggingMDNMLE(LightningModule):
    ...
    def __init__(
        ...
        if self.remove_last_layer == 0:
            if 'gpt' in huggingface_model.lower():
            ...
            if 'gpt' in huggingface_model.lower():
                print('using GPT without layers')
    ...
    def step(self, batch: Dict[str, torch.tensor], verbose: bool = False):
        ...
        labels = batch["tokenized_labels"].unsqueeze(2)
        ...
        use_multivariate_normal = True # Why was this false ??
        if use_multivariate_normal:
            ...
            dist = Normal(mu, var)
            # pdb.set_trace()
            log_likelihoods = dist.log_prob(labels)
            # Shape of log_likelihoods: [batch_size, seq_len, num_mixtures, num_labels]
            # But, with the code we only have [batch_size, seq_len, num_mixtures, num_labels]
